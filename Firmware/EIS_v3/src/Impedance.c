...
AppIMPCfg_Type AppIMPCfg = 
{
  ...
  // .ImpODR = 20,
  // .ImpODR = 0.000000000001,           /* 20.0 Hz*/
  // .ImpODR = 0.001,           // Good output rate for time - low magnitude still 
  .ImpODR = 0.01,           // Trying this out
  .NumOfData = -1,
...
static AD5940Err AppIMPSeqCfgGen(void)
{
  ...
  if(AppIMPCfg.SweepCfg.SweepEn == bTRUE)
  {
    ...
    logSweep(&AppIMPCfg.SweepCfg, &AppIMPCfg.SweepNextFreq);
...
static AD5940Err AppIMPSeqMeasureGen(void)
{
  ...
  printf("Clock cycles to wait for: %d\n", WaitClks);
  /* Sequence Generator Starts */
  AD5940_SEQGenCtrl(bTRUE);
